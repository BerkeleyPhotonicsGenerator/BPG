...
class PhotonicTemplateDB(TemplateDB):
    ...
    def to_lumerical(self,
        ...
        3. For each element in the flat content list, convert it into lsf code and append to running export file
        4. lsf code is generated by sending properties and tech info to the lsf_export static method in each shape class
        5. lsf code is appended to the running file with LumericalDesignGenerator
        ...
        use_dataprep : bool
            True to use the content list generated by dataprep, otherwise uses flat_content_list
        debug : bool
        ...
        if use_dataprep is True:
            # Run the lumerical dataprep routine
            self.dataprep(lsf_map, debug=debug)
            self.create_masters_in_db(lib_name='lsf_dp', content_list=self.post_dataprep_flat_content_list)
            content_list = self.post_dataprep_flat_content_list
        else:
            content_list = self.flat_content_list_separate

        # 3) For each element in the flat content list, convert it into lsf code and append to running file

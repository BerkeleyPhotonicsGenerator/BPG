...
#
#     """
#     clean up coordinates in the list that are redundant or harmful for following Shapely polygon manipulation
#
#     Parameters
#     ----------
...
#
#     #### if the last coord in the new list shares the x or y coordinate with
...
#         # print(i, coord_to_append, coord_1stlast, coord_2ndlast)
#         itm_share_x = apprx_equal(coord_1stlast[0], coord_2ndlast[0]) and apprx_equal(coord_1stlast[0], coord_to_append[0])
#         itm_share_y = apprx_equal(coord_1stlast[1], coord_2ndlast[1]) and apprx_equal(coord_1stlast[1], coord_to_append[1])
#         if (itm_share_x or itm_share_y):
...
#         first_share_x = apprx_equal(coords_list_out[0][0], coords_list_out[1][0]) and \
#                         apprx_equal(coords_list_out[0][0], coords_list_out[-2][0])
#         first_share_y = apprx_equal(coords_list_out[0][1], coords_list_out[1][1]) and \
#                         apprx_equal(coords_list_out[0][1], coords_list_out[-2][1])
#         if (first_share_x or first_share_y):
#             # print('sharex, sharey', first_share_x, first_share_y)
#             # print('poly_coords_orth_final', poly_coords_orth_final[:2], poly_coords_orth_final[-2:])
#             # print('should delete the first element')
...
def manh_skill(poly_coords,     # type: list[tuple(float, float)]
    ...
    if (manh_type == 'non'):
        poly_coords_manhgrid_final = coords_cleanup(poly_coords_manhgrid)
        return poly_coords_manhgrid_final
            ...
            if ((abs(delta_x) < eps_float) or (abs(delta_y) < eps_float)):
                ...
                for j in range(0, num_point_add):
                    ...
                    #  elseincrememnting Y first
                        ...
                        poly_coords_orth.append((x0+xstep, y0+ystep))
        #### clean up the coords
        poly_coords_orth_final = coords_cleanup(poly_coords_orth)
        return poly_coords_orth_final
    else:
        raise ValueError('manh_type = %s should be either "non", "inc" or "dec"' %(manh_type))



def manh_skill_pre(poly_coords,     # type: list[tuple(float, float)]
               manh_grid_size,  # type: float
               ):
...
def polyop_manh(geom,               # type: Polygon, MultiPolygon
    ...
    def polyop_manh_Polygon(geom,           # type: Polygon
        ...
        if (do_manh == True):
            coords_ext_manh = manh_skill(coords_ext_orig, manh_grid_size, manh_type='inc')
        else:
            coords_ext_manh = manh_skill(coords_ext_orig, manh_grid_size, manh_type='non')
        ...
        if (len(geom.interiors)):
            ...
            for interior in geom.interiors:
                ...
                # print(manh_type)
                if (do_manh == True):
                    coords_int_manh = manh_skill(coords_int_orig, manh_grid_size, manh_type='dec')
                else:
                    coords_int_manh = manh_skill(coords_int_orig, manh_grid_size, manh_type='non')

                poly_int_manh = Polygon(coords_int_manh).buffer(0, cap_style=3, join_style=2)

                if (interior == geom.interiors[0]):
                    poly_int_union_manh = Polygon(poly_int_manh)
                else:
                    poly_int_union_manh = poly_int_union_manh.union(poly_int_manh)
            # print(poly_ext_manh)
            # print(poly_ints_manh)
...
diamond_coords = [(8, 0), (12, 6), (8, 9), (4, 6), (8, 0)]
diamond_boundary = LineString(diamond_coords)
# diamond_coords_manh = manhattanize(diamond_coords, 0.6)
# diamond_boundary_manh = LineString(diamond_coords_manh)
# poly_origin = cascaded_union(polygonize_full(diamond_boundary))
# poly_origin = cascaded_union(polygonize_full(diamond_boundary_manh))
...
ring_origin = (Point(8.0, 8.0).buffer(32, resolution=16)).difference\
              (Point(0.00, 0.00).buffer(16, resolution=16).union
              (Point(16.00, 16.00).buffer(16, resolution=16).difference
...
#               (Point(15.00, 16.00).buffer(8, cap_style=3, join_style=2)))
# print(ring_origin)

manh_grid_size = [2, 1, 0.5, 0.25, 0.10]
